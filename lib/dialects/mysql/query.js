...
var Query = function(connection, sequelize, options) {
  ...
  this.connection.readOnlyErrors = _.get(this.connection, 'readOnlyErrors', 0);
  this.MAX_READONLY_ERRORS = _.get(this.options, 'pool.maxReadOnlyErrors', null);
...
Query.prototype.formatError = function (err) {
  ...
  switch (err.errno || err.code) {
    case 1290:
      match = err.message.match(/The MySQL server is running with the --read-only option so it cannot execute this statement/);

      if (match) {
        this.connection.readOnlyErrors++;
        if (this.MAX_READONLY_ERRORS && (this.connection.readOnlyErrors >= this.MAX_READONLY_ERRORS)) {
          this.sequelize.connectionManager.disconnect(this.connection);
        }
      }
